...
int main() {
  ...
  for (int caseNum = 0; caseNum < cases; caseNum++) {
    ...
    cin >> str;
    string dStr = str + str;
    build_sarray(dStr, sarray);
    compute_lcp(dStr, sarray, lcp);
    /*
    for (int i = 0; i < dStr.length(); i++) {
      cout << setw(3) << i << ": " << setw(2) << lcp[i] << ", " << sarray[i]
           << ' ' << dStr.substr(sarray[i], dStr.length() - sarray[i]) << endl;
    }
    */
    int pos;
    for (int i = 0; i < dStr.length(); i++) {
      if (sarray[i] < dStr.length() / 2) {
        pos = i;
        break;
      }
    }
    for (int i = pos + 1; i < dStr.length(); i++) {
      if (lcp[i] >= dStr.length() / 2 && sarray[i] < sarray[pos]) {
        pos = i;
      } else {
        break;
      }
    }
    cout << sarray[pos] + 1 << endl;
  }
  return 0;
}