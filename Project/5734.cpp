...
string firstDifference(string &code);
int ctoi(char c);
...
int main() {
  string code;
  while (cin && cin >> code) {
    string fd = firstDifference(code);
    fd = fd;
    string temp = fd + fd;
    int sarray[600000];
    // int lcp[300000];
    build_sarray(temp, sarray);
    // compute_lcp(fd, sarray, lcp);
    // string best = temp.substr(sarray[1], fd.length());
    for (int i = 0; i < fd.length(); i++) {
      cout << temp[i + sarray[1]];
    }

    /*
        for (int i = 0; i < fd.length(); i++) {
          cout << setw(3) << i << ": " << setw(2) << lcp[i] << ", "
               << fd.substr(sarray[i], fd.length() - sarray[i]) << endl;
        }
    */
    cout << endl;
  }
...
string firstDifference(string &code) {
  ...
  for (int i = 0; i < code.length(); i++) {
    result += to_string(
        (ctoi(code[(i + 1) % code.length()]) - ctoi(code[i]) + 8) % 8);
  }
  return result;
}
...
// Following functions are drom suffixarrray.cc
